...
const coupons = async (req, res) => {
  ...
  const filter = search
    ? {
        $or: [{ couponCode: { $regex: search, $options: "i" } }],
      }
    : {};
  ...
  try {
    ...
    const fetchedCoupons = await couponModel
      .find(filter)
      .skip(skip)
      .limit(limit)
      .sort({ _id: -1 });
    ...
    console.log("Error while rendering the coupon listing page", error);
...
const newCoupon = async (req, res) => {
  try {
    console.log(
      "entered into the new coupon adding function from the admin side"
    );
    const {
      couponCode,
      couponType,
...
const updateCouponPage = async (req, res) => {
  try {
    ...
    console.log(
      "error while rendering the updatecoupon page form the admin side",
      error
    );
    res.render("admin/servererror");
  }
