...
const dailySalesReport = async (req, res) => {
  try {
    ...
    const TotalAmount = dailyReport.reduce(
      (acc, curr) => acc + curr.totalSales,
      0
    );
    console.log("TotalAmount:", TotalAmount);
    const TotalSaleCount = dailyReport.reduce(
      (acc, curr) => acc + curr.totalOrderCount,
      0
    );
    console.log("TotalSaleCount:", TotalSaleCount);
    const TotalCouponAmount = dailyReport.reduce(
    ...
    if (req.query.format === "pdf") {
      ...
      res.setHeader(
        "content-Disposition",
        "attachment;filename=daily_sales_report.pdf"
      );
    ...
    console.log(
      "error occured while rendering the daily sales report page",
      error
    );
  ...
  try {
    ...
    if (req.query.format === "pdf") {
      ...
      res.render("admin/salesreport", {
        ...
        page: "weekly",
        TotalAmount: weeklyReport.reduce(
          (acc, curr) => acc + curr.totalSales,
          0
        ),
    ...
    console.log(
      "Error occured while rendering the weekly sales report page",
      error
    );
    res.render("admin/servererror");
  }
...
const yearlySalesReport = async (req, res) => {
  try {
    ...
    if (req.query.format === "pdf") {
      ...
      res.setHeader(
        "content-Disposition",
        "attachment;filename=yearly_sales_report.pdf"
      );
      res.setHeader("content-Length", pdfBuffer.length);
      res.contentType("application/pdf");
      res.send(pdfBuffer);
      ...
      res.render("admin/salesreport", {
        ...
        page: "yearly",
        TotalAmount: yearlyReport.reduce(
          (acc, curr) => acc + curr.totalSales,
          0
        ),
...
const customDateSort = async (req, res) => {
  try {
    ...
    if (req.query.format === "pdf") {
      ...
      res.setHeader(
        "content-Disposition",
        "attachment;filename=custom_sales_report.pdf"
      );
      res.setHeader("content-Length", pdfBuffer.length);
      res.contentType("application/pdf");
      res.send(pdfBuffer);
      ...
      res.render("admin/salesreport", {
        ...
        page: "daily",
        TotalAmount: customReport.reduce(
          (acc, curr) => acc + curr.totalSales,
          0
        ),
...
const checkDataExist = async (req, res) => {
  try {
    console.log(
      "entering into the function checking the data existance in the specified date range"
    );
...
const generateSalesReportPdf = (
  ...
  return new Promise((resolve, reject) => {
    ...
    drawTableRow(
      doc,
      table,
      ["Date", "Orders", "Sales", "Products", "Offered", "Coupons"],
      true
    );
    // Draw table rows
  ...
  function drawTableRow(doc, table, rowData, isHeader = false) {
    ...
    rowData.forEach((text, i) => {
      // Draw vertical line
      doc
        .moveTo(table.x + i * table.columnWidth, table.y)
        .lineTo(table.x + i * table.columnWidth, table.y + table.rowHeight)
        .stroke();

      // Add cell content
      doc
        .fillColor(isHeader ? "black" : "gray")
    ...
    doc
      .moveTo(table.x + table.width, table.y)
    ...
    if (table.y + table.rowHeight > doc.page.height - 50) {
      ...
      doc
        .moveTo(table.x, table.y + table.rowHeight)
        .lineTo(table.x + table.width, table.y + table.rowHeight)
        .stroke();
    }
