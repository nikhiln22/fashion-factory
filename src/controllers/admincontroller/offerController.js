...
const offerType = async (req, res) => {
  try {
    console.log(
      "entering to the function where we choose which is the offertype"
    );
    ...
    if (offerType === "category") {
      ...
      res.status(HTTP_STATUS.OK).json({ categoryDetails });
    } else if (offerType === "product") {
      const productDetails = await productModel.find({});
      console.log("productDetails:", productDetails);
      res.status(HTTP_STATUS.OK).json({ productDetails });
    }
  } catch (error) {
  ...
  try {
    ...
    if (!offerName || !offerName.trim()) {
      return res
        ...
        .json({ success: false, message: "Please enter an offer name" });
    }
    ...
    if (!startDate) {
      return res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json({ success: false, message: "Please enter a start date" });
    }

    if (!endDate) {
      return res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json({ success: false, message: "Please enter an end date" });
    }

    if (!offerType || offerType === "choose") {
      return res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json({ success: false, message: "Please select a valid offer type" });
    }
    ...
    console.error(
      "Error while adding an offer for the category or product",
      error
    );
...
const editOfferPage = async (req, res) => {
  try {
    ...
    if (offerData.productId.length > 0) {
      ...
      res.render("admin/editoffer", {
        offerData,
        Details: catagoryData,
        activePage: "offer",
      });
    } else {
      res.status(HTTP_STATUS.BAD_REQUEST).render("admin/servererror");
    }
  } catch (error) {
    console.log("error occured while rendering the offer editing page", error);
    res.status(HTTP_STATUS.SERVER_ERROR).render("admin/servererror");
  }
};
...
const offerProdIdSave = async (req, res) => {
  try {
    console.log(
      "entering to the offerproductId saving function from offer controller"
    );
    ...
    if (selectedProductIds.length) {
      await offerModel.updateOne(
        { _id: offerId },
        { $set: { productId: selectedProductIds } }
      );
    }
...
const offercatIdSave = async (req, res) => {
  try {
    console.log(
      "entering to the offercategoryId saving function from offer controller"
    );
    ...
    if (selectedCategoryIds.length) {
      await offerModel.updateOne(
        { _id: offerId },
        { $Set: { categoryId: selectedCategoryIds } }
      );
    }
    res.json({ success: true });
  } catch (error) {
...
const editoffer = async (req, res) => {
  try {
    console.log("entering the edit offer from the offer controller");
    ...
    if (!startDate || !endDate) {
      return res.json({
        success: false,
        message: "start and End date is needed",
      });
    }
...
const deleteOffer = async (req, res) => {
  try {
    console.log(
      "entering into deleting the selected offer from the admin side"
    );
    const { id } = req.body;
