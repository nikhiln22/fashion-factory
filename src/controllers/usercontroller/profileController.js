...
const editAddressPost = async (req, res) => {
  try {
    ...
    const isAddressExists = await addModel.findOne({
      userId,
      "address._id": { $ne: new ObjectId(addressId) },
  ...
  try {
    ...
    if (!orderDetails) {
      return res
        ...
        .render("error", { error: "Order not found." });
    }
...
const cancelProduct = async (req, res) => {
  try {
    ...
    const itemIndex = orderdetails.orderedItem.findIndex(
    ...
    if (item.productStatus === "cancelled") {
      return res
        ...
        .json({ success: false, message: "Product is already cancelled" });
    }
    ...
    if (orderdetails.couponDiscount) {
      refundAmount =
        ...
        orderdetails.couponDiscount / totalOrderedProducts;
...
const returnOrder = async (req, res) => {
  try {
    ...
    if (
      item.productStatus === "return initiated" ||
      item.productStatus === "returned"
    ) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({
        success: false,
        message: "Return already initiated or completed for this product",
      });
    }
...
// adding the money to the wallet
...
const invoicePage = async (req, res) => {
  try {
    console.log(
      "rendering the invoice page for the user displaying ordered product details"
    );
    ...
    if (!orderId || !productId) {
      return res
        .status(HTTP_STATUS.BAD_REQUEST)
        .json({ error: "Missing orderId or productId" });
    }

    const orderDetails = await orderModel
      .findOne({ _id: orderId.trim() })
      .populate("orderedItem.productId");
    console.log("orderDetails:", orderDetails);
    ...
    const productData = orderDetails.orderedItem.find(
      (item) => item.productId._id.toString() === productId
    );
