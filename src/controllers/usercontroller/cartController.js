  ...
  try {
    ...
    const insufficientStock = cartItemsWithOffers
      ...
      .map((cartItem) => ({
        item: cartItem,
        availableQuantity:
          cartItem.productId.stock.find((s) => s.size == cartItem.size)
            ?.quantity || 0,
      }));
  ...
  try {
    ...
    activeOffers.forEach((offer) => {
      if (offer.offerType === "product" && offer.productId.includes(pid)) {
        console.log(
          "potential discount calculating if any product offer exists"
        );
        const potentialDiscountPrice =
    ...
    if (!appliedOffer) {
      activeOffers.forEach((offer) => {
        if (
          offer.offerType === "category" &&
          offer.categoryId.includes(product.category.toString())
        ) {
          console.log(
            "potential discount calculating if any category offer exists"
          );
          const potentialDiscountPrice =
            product.price - (product.price * offer.discount) / 100;
          console.log("potentialDiscountPrice:", potentialDiscountPrice);
    ...
    if (!stock) {
      return res.json({
        success: false,
        message: "Selected size not found for this product.",
      });
    }
    ...
    if (!selectedStock || selectedStock.quantity === 0) {
      return res.json({
        success: false,
        message: "Selected size is out of stock. Please choose another size.",
      });
    }

    let cart =
      (await cartModel.findOne({ userId })) ||
      (await cartModel.findOne({ sessionId: req.session.id }));
    ...
    if (productExist !== -1) {
      ...
      cart.item[productExist].total =
        cart.item[productExist].quantity * discountedPrice;
...
const updateCart = async (req, res) => {
  try {
    ...
    const newProductTotal =
      (offerPrice < price ? offerPrice : price) * updatedQuantity;
    ...
    const newCartCount = cart.item.reduce(
...
const deleteCart = async (req, res) => {
  try {
    console.log(
      "entering into function that removing the products from the cart"
    );
    const userId = req.session.userId;
    const pid = req.params.id;
    const size = req.params.size;
    const output = await cartModel.updateOne(
      { userId: userId },
