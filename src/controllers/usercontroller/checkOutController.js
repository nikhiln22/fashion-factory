...
const checkout = async (req, res) => {
  try {
    ...
    const validCoupons = couponDetails.filter(
      (coupon) =>
...
const applyCoupon = async (req, res) => {
  try {
    ...
    if (couponData && couponData.status === true) {
      ...
      if (
        couponData.expiry > new Date() &&
        couponData.minimumPrice <= cartTotal
      ) {
        let discountPrice;
        let totalAmount;
...
const removeCoupon = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: "An error occured while removing the coupon",
      });
  }
};
...
const retryPayment = async (req, res) => {
  try {
    console.log(
      "entering into the retrying the failed razorpay payment while placing an order"
    );
    const userId = req.session.userId;
...
const orderConfirmation = async (req, res) => {
  try {
    ...
    console.log(
      "error in rendering the order confirmation page for the user",
      error
    );
    res.render("user/error");
  }
